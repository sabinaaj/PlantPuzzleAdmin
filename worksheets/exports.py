...
from .models import Worksheet, TaskType, Task
...
class WorksheetExportView(View):
    def get(self, request, *args, **kwargs):
        worksheet = get_object_or_404(Worksheet, pk=kwargs['pk'])

        # Načítání úkolů a obrázků
        tasks = worksheet.task_set.all()
        image = Task.objects.filter(image__isnull=False).first().image

        # Vytvoření kontextu pro šablonu
        context = {
            'worksheet': worksheet,
            'tasks': tasks,
        ...
        # Render šablony do HTML
        html_content = render_to_string('new_template.html', context)

        # Vytvoření odpovědi pro PDF
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename=pracovni_list_{worksheet.pk}.pdf'

        # Konverze HTML na PDF
        pdf = pdfkit.from_string(html_content, False, options=options)
        response.write(pdf)

        return response

# class WorksheetExportView(WeasyTemplateResponseMixin, DetailView):
#     model = Worksheet
#     template_name = 'new_template.html'
#     pdf_filename = 'pracovni_list.pdf'
#
...
#
#         # Získání všech úkolů a obrázků
#         tasks = self.worksheet.task_set.all()
#         image = Task.objects.filter(image__isnull=False).first().image
#
#         # Přidání úkolů a obrázku do kontextu
#         context['tasks'] = tasks
#         context['image'] = image.path
#         return context
...
#
#         # Pokud se objeví nějaké chyby, vrátí se status 500
#         if pisa_status.err:
...
#         pdf_file = BytesIO()
#         HTML(string=html_content.content.decode(), base_url=request.build_absolute_uri('/')).write_pdf(pdf_file)
#         pdf_file.seek(0)
#
